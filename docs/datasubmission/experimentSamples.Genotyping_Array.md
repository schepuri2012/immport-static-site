# Template: **genotyping_array**

| Name | Value |
| --- | --- |
| Template Type | combined-multiple |
| Schema Version | 3.33 |
| Standard File Name | experimentSamples.Genotyping_Array.txt |
...
| Repository Name | repository_name | ImmPort expects genotyping results to be deposited in dbGAP since this is a prerequisite for publication. In order to avoid duplication of data upload by requiring the same data be sent to ImmPort as well as dbGAP, ImmPort requires only the dbGAP accession. | Genotyping results are expected to be deposited in dbGAP. Please choose this repository name from the list. |
